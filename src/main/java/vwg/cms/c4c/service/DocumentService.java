...
public class DocumentService {
    ...
    public DocumentResponse submitDocument(Long id, SubmitDocumentRequest request, String actorUsername) {
        ...
        notifyReviewerTarget(reviewerUsername, document.getTitle(), document.getId(),
    ...
    public DocumentResponse reviewDocument(Long id, ReviewDecisionRequest request, String actorUsername) {
        ...
        String creatorUsername = findCreatorUsername(id);
    ...
    private String findCreatorUsername(Long documentId) {
        return approvalRepository.findByDocumentIdOrderByCreatedAtDesc(documentId).stream()
                .filter(approval -> approval.getAction() == ApprovalAction.CREATED)
                .map(Approval::getActorUsername)
                .findFirst()
                .orElse(null);
    }

    private String resolveReviewerTarget(String identifier) {
        if (identifier == null || identifier.isBlank()) {
            return null;
        }
        DistributionList distributionList = distributionListService.findByEmail(identifier.trim());
        return distributionList != null
                ? distributionList.getEmail()
                : appUserService.resolveReviewerUsername(identifier);
    }

    private void notifyReviewerTarget(String target, String title, Long documentId, String message) {
        if (target == null) {
            return;
        }
        DistributionList distributionList = distributionListService.findByEmail(target);
        if (distributionList != null) {
            distributionList.getMemberEmails().forEach(memberEmail ->
                    appUserService.findUsernameByEmail(memberEmail).ifPresent(username ->
                            notificationService.notifyUser(username, NotificationType.REVIEW_REQUEST, message, documentId, reviewDueAt())));
            return;
        }
    ...
    private void ensureReviewer(AppUser actor) {
        if (!appUserService.isReviewer(actor)) {
            throw new ForbiddenOperationException("Only SDM, PD Head, Sub Admin, or Admin users can review documents");
        }
    }
    ...
    private boolean canActOnReview(Document document, AppUser actor) {
        ...
        DistributionList distributionList = distributionListService.findByEmail(assignedReviewer);
        if (distributionList != null) {
            return distributionList.getMemberEmails().stream()
    ...
    private int roleRank(vwg.cms.c4c.model.Role role) {
        return switch (role) {
            case SDM -> 1;
            case PD_HEAD -> 2;
            case SUB_ADMIN -> 3;
