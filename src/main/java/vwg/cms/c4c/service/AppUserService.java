...
public class AppUserService implements UserDetailsService {
    ...
    public Optional<String> findUsernameByEmail(String email) {
        return appUserRepository.findByEmailIgnoreCase(email).map(AppUser::getUsername);
    }
    ...
    public String resolveReviewerUsername(String identifier) {
        if (identifier == null || identifier.isBlank()) {
            return null;
        }
        AppUser reviewer = identifier.contains("@")
                ? appUserRepository.findByEmailIgnoreCase(identifier.trim()).orElseThrow(
                        () -> new ResourceNotFoundException("No user was found for reviewer email: " + identifier))
                : getRequiredUser(identifier.trim());
        if (!isReviewer(reviewer)) {
            throw new BadRequestException("The email belongs to a " + reviewer.getRole()
                    + " user. Please enter an SDM, PD Head, or Admin email.");
        }
    ...
    public boolean canManageDistributionLists(AppUser user) {
        return user.getRole() == Role.ADMIN || user.getRole() == Role.SUB_ADMIN;
    }

    public boolean isReviewer(AppUser user) {
        return user.getRole() == Role.ADMIN
            || user.getRole() == Role.SUB_ADMIN
            || user.getRole() == Role.SDM
            || user.getRole() == Role.PD_HEAD;
    }
