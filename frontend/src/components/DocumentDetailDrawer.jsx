...
function canReview(role) {
  return ['SDM', 'PD_HEAD', 'SUB_ADMIN', 'ADMIN'].includes(role);
}
...
export default function DocumentDetailDrawer({ open, documentId, user, api, onClose, onChanged, showMessage, includeDeleted = false, reviewerOptions = [] }) {
  ...
  const canStartReview = !isDeleted
    && reviewerRole
    && document?.createdBy !== user?.username
    && document?.status === 'SUBMITTED';
  const canApproveReject = !isDeleted
    && reviewerRole
    && document?.createdBy !== user?.username
  ...
  const assignedReviewer = reviewerOptions.find((option) => option.username === document?.assignedReviewer);
              ...
              <Box sx={{ p: 2, bgcolor: 'background.paper', borderRadius: 3 }}>
                <Stack spacing={1}>
                  ...
                  <Typography>
                    ...
                    {assignedReviewer
                      ? `${assignedReviewer.email} (${assignedReviewer.role})`
                      : document.assignedReviewer || 'Unassigned'}
                  </Typography>
